...
const isSaving = computed(() => {
    if (!store || typeof store.isFieldLoading !== 'function') return false;
    store.isFieldLoading(props.todoId, props.category)
});
...
const bgClass = computed(() => {
    return getColorClass(localValue.value, props.category) || 'bg-slate-700';
});
</script>
